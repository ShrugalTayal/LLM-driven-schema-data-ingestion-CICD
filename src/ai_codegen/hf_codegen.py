...
def gen_from_schema(schema_json: dict):
    ...
    def fallback():
        ...
        parts = []
        for c in cols:
            line = f'{c["name"]} {c.get("data_type","TEXT")}'
            if not c.get("is_nullable", True): line += " NOT NULL"
            parts.append(line)
        ddl = f'CREATE TABLE IF NOT EXISTS {schema_json["table_name"]} (\n  ' + ",\n  ".join(parts) + "\n);"
...
def ingest(source):
    ...
    df.to_sql('{schema_json["table_name"]}', engine, if_exists='append', index=False)
...
"""
    ...
    try:
        ...
        # resp can be dict/list - try to extract generated text
        text = ""
